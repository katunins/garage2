    ...
    <div class="input-blocks">
        <div class="input-group">
            ...
            <input class="max-size" type="text" id="params" name="params"
                value="{{ old('params', $template->params ?? '') }}"
                placeholder="Тип печати/Форзац">
            ...
            <div class="master-buttons">
                <div>
                    ...
                    <input class="mini-size" type="text" id="masters" name="masters"
                        value="{{ old('masters', $template->masters ?? '') }}"
                        placeholder="1/2" readonly>
                </div>
                <div class="all-masters-buttons">
                    ...
                    @foreach ($Users as $item)
                        ...
                        <button type="button"
                            class="@if (array_search($item->id, $mastersArr) !==false) half-opacity @endif"
                            id="{{ $item->id }}" onclick="clickMasterButtons(this)">
                            + {{ $item->name }}
                        </button>
                    @endforeach
                    <button type="button" onclick="document.getElementById('masters').value=''"
                        style="background-color: rgb(255, 124, 124)">Очистить все</button>
                </div>
            </div>
...
<script>
    ...
    function clickMasterButtons(elem) {
        ...
        if (elem.classList.contains('half-opacity')) {
            // удаляем  мастера
            elem.classList.remove('half-opacity')
            ...
            // добавляем мастера
            elem.classList.add('half-opacity')
